...
import com.xc.process.mapper.OaProcessMapper;
...
import static com.xc.process.utils.RandomTwenty.generateRandomNumberString;
...
//    private MessageService messageService;
    ...
    public void startUp(ProcessFormVo processFormVo) {
        //1 根据当前用户id获取用户信息
        DsUserEntity sysUser = dsUserService.getById(LoginUserInfoHelper.getUserId());
        ...
        for(Task task : taskList) {
            ...
            DsUserEntity user = dsUserService.getUserByUserName(assigneeName);
    ...
    public void approve(ApprovalVo approvalVo)  {
        ...
        if(!CollectionUtils.isEmpty(taskList)) {
            ...
            for(Task task : taskList) {
                ...
                DsUserEntity sysUser = dsUserService.getUserByUserName(assignee);
                assignList.add(sysUser.getUsername());
            ...
            if(approvalVo.getStatus().intValue() == 1) {
                ...
                if(template.getName().equals("报账")){
                    Gson gson = new Gson();
                    Map<String, Object> data = gson.fromJson(process.getFormValues(), Map.class);
                    Map<String, Object> formData = (Map<String, Object>) data.get("formData");

//                    随机生成15位的数
                    String SnOrder = generateRandomNumberString(15);

                    // 自动从支付宝进行扣款
//                    Result transfer = transferService.transfer((String) formData.get("account"), (String) formData.get("name"), (String) formData.get("price"),SnOrder);
//                    if(transfer.getCode()!=200){
//                        throw new ERPException(201,"转账失败"+transfer.getData());
//                    }
                }
