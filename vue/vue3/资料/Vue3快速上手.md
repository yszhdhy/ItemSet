...
<script lang="ts" setup name="Person">
  ...
  // 监视，情况一：监视【ref】定义的【基本类型】数据 监视sum的值
...
<script lang="ts" setup name="Person">
  ...
  /* 
    ...
    immediate:true 表示的就是立即监视 一上来就进行监视。如果监视中书写了打印语句就会直接进行打印一次。
  */
...
<script lang="ts" setup name="Person">
  ...
  // 监视，情况三：监视【reactive】定义的【对象类型】数据，且默认是开启深度监视的,且是无法进行关闭的
  watch(person,(newValue,oldValue)=>{
    console.log('person变化了',newValue,oldValue)
  })
...
![image-20240413100434766](./Vue3快速上手.assets/image-20240413100434766.png)
...
```js
{
  ...
  // props的布尔值写法，作用：把收到了每一组params参数，作为props传给Detail组件，但是仅能在params中进行使用，如果使用的是query 就要使用下面的方式进行
  // props:true
...
接收： 直接使用 defineprops进行接收数据即可 （也就是父组件传递给子组件的数据形式）
...
## 4.10. 【 replace属性】
  ...
  3. 开启`replace`模式：
     ...
     ![image-20240414193437900](./Vue3快速上手.assets/image-20240414193437900.png)
...
<!-- 将 news传递过去  下面的函数就可以接收到-->
 <button @click="showNewsDetail(news)">查看新闻</button> 

// 以下就是编程式路由导航的 引入 跳转 带参
...
## ![image-20240414194047521](./Vue3快速上手.assets/image-20240414194047521.png)
...
3. 第三种修改方式  在pinia中进行修改：借助`action`修改（`action`中可以编写一些业务逻辑）
...
在组件中 通过 store 的 `$subscribe()` 方法侦听 `state` 及其变化
...
2. 具体说明：`$attrs`是一个对象，包含所有父组件传入的标签属性。
   ...
   **也就是父组件传递数据给子组件 如果子组件没有进行接收 这些数据的话就会存放在 $attr中 。**
...
<template>
	<div class="child">
		<h3>子组件</h3>
        <!-- 子组件直接将父组件传递的props中的数据页通过props的形式 传递给了它的子组件中 -->
...
4. 具体编码：
   ...
   <script setup lang="ts" name="Father">
     ...
     // 提供数据    updateMoney  这个函数是为了实现 孙给父传递数据
   ...
   <script setup lang="ts" name="GrandChild">
     ...
     // 注入数据  第二个参数表示的就是默认值 如果我们没有接收到 moneyContext 就会显示默认值 0   ，updateMoney 这个函数的就是 进行操作爷组件
...
子组件中：  之后父组件中的 ul等数据就会显示 在slot标签中。这就是插槽的作用
        <template>
          <div class="item">
            <h3>{{ title }}</h3>
            <!-- 默认插槽 -->
...
3. 具体编码：
   ...
   ```vue
   父组件中： 父组件通过 v-slot="params" 的形式接收到子组件传递过来的数据
         <Game v-slot="params">
         ...
         <template>
           <div class="category">
             <h2>今日游戏榜单</h2>
               ...
               相当与将 子组件中的数据通过props 的形式传递给父组件中  只是父组件接收方式不一样
...
- `app.component`
- ![image-20240416194455920](./Vue3快速上手.assets/image-20240416194455920.png)
- `app.config`![image-20240416194556651](./Vue3快速上手.assets/image-20240416194556651.png)
- `app.directive`
- `app.mount`
- `app.unmount`
- `app.use`
